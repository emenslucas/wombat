...
import { Play, Pause, SkipBack, SkipForward, Volume2, VolumeX, Volume1, Repeat, Shuffle } from 'lucide-react'
...
export function AudioPlayer({
  ...
  const handleNext = useCallback(() => {
    if (isShuffle) {
      ...
      onTrackChange(randomIndex !== currentTrackIndex ? randomIndex : (randomIndex + 1) % tracks.length)
    } else {
            ...
            <div className="flex items-center gap-1 md:gap-2">
              <button
                ...
                className={cn(
                  ...
                  isShuffle ? "text-foreground" : "text-muted-foreground/50 hover:text-muted-foreground"
                )}
              ...
              <button
                ...
                className={cn(
                  "hidden md:flex p-2 transition-colors duration-200",
                  isRepeat ? "text-foreground" : "text-muted-foreground/50 hover:text-muted-foreground"
                )}
                aria-label="Repetir"
              >
...
export function TrackList({ tracks, currentTrackIndex, onTrackSelect, isPlaying }: TrackListProps) {
  return (
    <div className="rounded-xl overflow-hidden bg-card/30 ring-1 ring-border/30">
      {tracks.map((track, index) => (
        <button
          ...
          className={cn(
            'w-full flex items-center gap-4 px-4 md:px-5 py-3 md:py-4 text-left transition-all duration-200 group',
            'hover:bg-accent/30',
            index === currentTrackIndex && 'bg-accent/20',
            index !== tracks.length - 1 && 'border-b border-border/20'
          )}
        >
          <span className="w-8 text-center text-sm text-muted-foreground/60 tabular-nums font-mono group-hover:hidden">
          ...
          <div className="flex-1 min-w-0">
            <p className={cn(
              ...
              index === currentTrackIndex ? 'text-foreground font-medium' : 'text-foreground/80'
            )}>
              {track.title}
            </p>
          </div>
